...
export class TopNavComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  private loadSettings() {
    this.subscriptions.push(
      this.auth.AuthStatusSubject.subscribe((r: any) => {
        this.loggedIn = r;
      })
    );
    this.subscriptions.push(
      this.layoutUtils.getCartChangeObservable().subscribe((r: any) => {
  ...
  public handleOutsideClick(e) {
    ...
    if (e.target.offsetParent.id == 'menuTwoToggler') return;
    this.megaMenuTwoOpen = false;
  }

  public menuTwoToggle() {
    this.megaMenuTwoOpen = !this.megaMenuTwoOpen;
  }
}
